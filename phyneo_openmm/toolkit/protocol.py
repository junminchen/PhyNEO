...
try:
    ...
    dcd_read = npt_run = nvt_run = rescale_box = volume_calc = None
    onsager_calc = nonequ_run = viscosity_calc = None
    GMXScript = generate_openmm_system = get_nb_params = load_model = None
    Molecule = RecordAtomType = RecordMolecule = Records = TopoFullSystem = None
    def setup_default_logging():  # type: ignore
        import logging
...
def _require_pandas():
    if pd is None:
        raise RuntimeError("pandas is required for this post-process path but is not installed.")
...
def write_gro(mol: Molecule, save_path: str):
    if aio is None:
        raise RuntimeError("ase is required for write_gro but is not installed.")
...
class XMLProtocolAdapter:
    """
    Adapter entry for Packmol-PDB + XML workflows.
    Reuses the validated loading pipeline:
    - validate_inputs
    - load_phyneo_system
    - apply_mpid_scale_exclusions

    This keeps legacy `toolkit.protocol` as the integration entry while allowing
    direct PDB/XML protocols without topology/FF-generation stages.
    """

    def __init__(self, config: dict):
        self.config = config
        base_dir = self.config.get("_config_base_dir")
        if base_dir:
            base_dir = os.path.abspath(base_dir)
            input_cfg = self.config.get("input", {})
            output_cfg = self.config.get("output", {})
            if isinstance(input_cfg, dict):
                if input_cfg.get("pdb") and not os.path.isabs(input_cfg["pdb"]):
                    input_cfg["pdb"] = os.path.abspath(os.path.join(base_dir, input_cfg["pdb"]))
                if input_cfg.get("xml") and not os.path.isabs(input_cfg["xml"]):
                    input_cfg["xml"] = os.path.abspath(os.path.join(base_dir, input_cfg["xml"]))
            if isinstance(output_cfg, dict):
                if output_cfg.get("work_dir") and not os.path.isabs(output_cfg["work_dir"]):
                    output_cfg["work_dir"] = os.path.abspath(os.path.join(base_dir, output_cfg["work_dir"]))
        self.result = None

    def run_protocol(self):
        from phyneo_openmm.phyneo_protocol import run_protocol as run_phyneo_protocol

        self.result = run_phyneo_protocol(self.config)
        return self.result

    def post_process(self):
        if self.result is None:
            return {}
        return self.result.get("post_process", {})


def create_protocol_from_config(config: dict):
    """
    Unified factory for protocol selection.

    Selection rule:
    - if `config.input.pdb` and `config.input.xml` are both provided:
      return XMLProtocolAdapter (load_phyneo_system path)
    - otherwise fallback to legacy Protocol classes.
    """
    if isinstance(config, str):
        if os.path.exists(config):
            ...
            with open(cfg_path, "r") as f:
                config = json.load(f)
            config["_config_base_dir"] = os.path.dirname(cfg_path)
        else:
    ...
    if isinstance(input_cfg, dict) and input_cfg.get("pdb") and input_cfg.get("xml"):
        return XMLProtocolAdapter(config)

    ptype = str(config.get("protocol_type", "transport")).lower()
    if ptype == "density":
        return DensityProtocol(config)
    if ptype == "hvap":
        return HVapProtocol(config)
    return TransportProtocol(config)
