...
def run():
    ...
    pdb_path = str(example_root / 'ec_box_35A.pdb')
    xml_path = str(example_root / 'caff_5_mpid_slater_bond.xml')
    ...
    # Connectivity: (0,1), (1,2), (1,5), (2,3), (3,4), (4,5), (3,6), (3,7), (4,8), (4,9)
    # relative to residue start.
    ...
    # [1-2, 1-3, 1-4, 1-5, >1-5/all]
    # Set last element to 1.0 to avoid excluding ALL intra-residue pairs (which would kill 1-6+)
    ...
    # We must synchronize exclusions if there are any CustomNonbondedForces, 
    # as they must match MPIDForce (or NonbondedForce if present).
    if custom_forces_indices:
        print(f"Rigidly synchronizing exclusions for CUDA compatibility...")
        
        # 1. Collect UNION of all exclusions
        final_exclusions = set()
        if nb_force:
            for i in range(nb_force.getNumExceptions()):
                p1, p2, q, sig, eps = nb_force.getExceptionParameters(i)
                # Only count as exclusion if it's meant to be excluded (eps=0, q1*q2=0)
                # But CUDA wants *identical lists*, so we take all exceptions.
        ...
        # 2. Add m_scales logic to the union (to match MPIDForce)
        ...
        # 3. Update NonbondedForce Exceptions
        if nb_force:
            # Map existing exceptions to avoid duplicates when adding final_exclusions
            existing_pairs = {}
            for i in range(nb_force.getNumExceptions()):
            ...
            for p1, p2 in final_exclusions:
                ...
                if pair in existing_pairs:
                    # Update existing to be an exclusion (0 interaction)
        ...
        # 4. Rebuild CustomNonbondedForces to ensure pure identical lists
        # Since OpenMM doesn't have clearExclusions, we replace the force.
    ...
    # Assign force groups for energy breakdown debugging
    force_groups = {}
    ...
    print("Minimizing energy (heavier)...")
    sim.minimizeEnergy(maxIterations=500)
    
    num_steps = 100000
    report_interval = 1000
    print(f"Starting simulation ({num_steps} steps, interval={report_interval}) on {platform.getName()} platform...")
