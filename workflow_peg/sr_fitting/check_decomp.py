...
if __name__ == '__main__':
    ff = 'forcefield.xml'
    pdb_AB = PDBFile('cocco_dimer.pdb')
    pdb_A = PDBFile('cocco.pdb')
    pdb_B = PDBFile('cocco.pdb')
    ...
    ofile_tot = open('res_tot.txt', 'w')
    ofile_es = open('res_es.txt', 'w')
    ofile_ex = open('res_ex.txt', 'w')
    ofile_pol = open('res_pol.txt', 'w')
    ofile_disp = open('res_disp.txt', 'w')
    ofile_dhf = open('res_dhf.txt', 'w')
    ofile_decomp = open('res_decomp.txt', 'w')
    # run test
    for sid in sids:
        # print(sid)
    # for sid in ['000']:
        ...
        for ipt in range(npts):
            ...
            E_B = pot_pme_B(pos_B, box, pairs_B, paramtree)
            ...
            E_es_tot = E_es + E_sr_es + E_dmp_es
            E_pol_tot = E_pol + E_sr_pol
            E_disp_tot = E_disp + E_dmp_disp + E_sr_disp
            ...
            if E_tot_ref < 30:
                ...
                print(ipt, E_tot, E_tot_ref, E_es_tot, E_es_ref, E_ex, E_ex_ref, E_pol_tot, E_pol_ref, E_disp_tot, E_disp_ref, E_dhf, E_dhf_ref, file=ofile_decomp)

            if E_tot_ref < 30: # E_tot_ref < 30
                print(E_tot_ref, E_tot, E_tot_ref)
