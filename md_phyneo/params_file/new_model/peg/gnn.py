...
def LayerNorm(f_in, weight, bias, axis=-1, eps=1e-5):
    mean = jnp.mean(f_in, axis=-1, keepdims=True)
    var = jnp.var(f_in, axis=-1, keepdims=True)
    std = jnp.sqrt(var + eps)
    f_out = (f_in - mean) / std * weight + bias
    return f_out
...
class MolGNNForce:
    ...
    def __init__(self,
        ...
        for i_nn, n_layers in enumerate(n_layers):
            ...
            params[nn_name + '.bias'] = []
            params[nn_name + '.scale'] = []
            params[nn_name + '.offset'] = []
            for i_layer in range(n_layers):
                ...
                params[nn_name + '.scale'].append(jnp.ones(dim_out))
                params[nn_name + '.offset'].append(jnp.zeros(dim_out))
                dim_in = dim_out
        key, subkey = jax.random.split(key)
        ...
        def forward(positions, box, params, nn):
            ...
            def fc0(f_in, params):
                ...
                for i in range(self.n_layers[0]):
                    f = params['fc0.weight'][i].dot(f) + params['fc0.bias'][i]
                    f = LayerNorm(f, params['fc0.scale'][i], params['fc0.offset'][i])
                    f = jnp.tanh(f)
                return f
            ...
            def fc1(f_in, params):
                ...
                for i in range(self.n_layers[1]):
                    f = params['fc1.weight'][i].dot(f) + params['fc1.bias'][i]
                    f = LayerNorm(f, params['fc1.scale'][i], params['fc1.offset'][i])
                    f = jnp.tanh(f)
                return f
        ...
        for i_nn in [0, 1]:
            ...
            for k in keys:
                if re.search(nn_name + '.[0-9]+.weight', k) is not None:
                    ...
                    keys_bias.append(k)
                elif re.search(nn_name + '.[0-9]+.scale', k) is not None:
                    keys_scale.append(k)
                elif re.search(nn_name + '.[0-9]+.offset', k) is not None:
                    keys_offset.append(k)
            ...
            params['%s.scale' % nn_name] = []
            params['%s.offset' % nn_name] = []
            for i_layer in range(self.n_layers[i_nn]):
                k_w = '%s.%d.weight' % (nn_name, i_layer)
                k_b = '%s.%d.bias' % (nn_name, i_layer)
                k_s = '%s.%d.scale' % (nn_name, i_layer)
                k_o = '%s.%d.offset' % (nn_name, i_layer)
                params['%s.weight' % nn_name].append(params.pop(k_w, None))
                params['%s.bias' % nn_name].append(params.pop(k_b, None))
                params['%s.scale' % nn_name].append(params.pop(k_s, None))
                params['%s.offset' % nn_name].append(params.pop(k_o, None))
            # params[nn_name]
        self.params = params
        return
            ...
            for i_layer in range(self.n_layers[i_nn]):
                ...
                params[nn_name +
                       '.%d.bias' % i_layer] = self.params[nn_name +
                                                           '.bias'][i_layer]
                params[nn_name + '.%d.scale' %
                       i_layer] = self.params[nn_name + '.scale'][i_layer]
                params[nn_name +
                       '.%d.offset' % i_layer] = self.params[nn_name +
                                                           '.offset'][i_layer]
